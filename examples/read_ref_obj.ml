...
let () =
  ...
  let dataset = H5d.open_ (H5f.to_loc fid1) "/Dataset3" in
  H5d.read dataset H5t.std_ref_obj H5s.all H5s.all
    (H5r.Hobj_ref.Bigarray.to_genarray rbuf);
  let dset2 = H5d.of_loc (H5r.Hobj_ref.(dereference (H5d.to_loc dataset)
    (Bigarray.unsafe_get rbuf 0))) in
  ...
  Printf.printf "\n";
  H5d.close dset2;
  let group = H5g.of_loc (H5r.Hobj_ref.(dereference (H5d.to_loc dataset)
    (Bigarray.unsafe_get rbuf 2))) in
  let read_comment = H5g.get_comment (H5g.to_loc group) "." in
  Printf.printf "Group comment is %s \n" read_comment;
  Printf.printf " \n";
  H5g.close group;
  let tid1 = H5t.of_loc (H5r.Hobj_ref.(dereference (H5d.to_loc dataset)
    (Bigarray.unsafe_get rbuf 3))) in
