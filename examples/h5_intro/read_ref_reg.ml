...
let () =
  ...
  let dset1 = H5d.open_ (H5f.to_loc fid1) "/Dataset1" in
  H5d.read dset1 H5t.std_ref_dsetreg H5s.all H5s.all
    (H5r.Hdset_reg_ref.Bigarray.to_genarray rbuf);
  let dset2 = H5d.of_loc (H5r.Hdset_reg_ref.(dereference (H5d.to_loc dset1)
  ...
  let sid2 = H5r.Hdset_reg_ref.(get_region (H5d.to_loc dset1)
    (Bigarray.unsafe_get rbuf 0)) in
