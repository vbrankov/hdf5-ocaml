...
let () =
  ...
  let dset2 = H5d.create (H5f.to_loc fid1) "Dataset2" H5t.std_u8le sid2 in
  ...
  let dset1 = H5d.create (H5f.to_loc fid1) "Dataset1" H5t.std_ref_dsetreg sid1 in
  H5s.select_hyperslab sid2 H5s.Select.SET ~start:[| 2; 2 |] ~stride:[| 1; 1 |]
    ~count:[| 6; 6 |] ~block:[| 1; 1 |] ();
  H5r.Hdset_reg_ref.(Bigarray.unsafe_set wbuf 0
    (create (H5f.to_loc fid1) ~space:sid2 "/Dataset2"));
  H5s.select_elements sid2 H5s.Select.SET [|
    ...
    7; 1;
    3; 3 |];
  H5r.Hdset_reg_ref.(Bigarray.unsafe_set wbuf 1
    (create (H5f.to_loc fid1) ~space:sid2 "/Dataset2"));
