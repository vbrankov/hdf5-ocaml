...
let () =
  ...
  let dataset = H5d.create (H5f.to_loc file) _DSET_VLSTR_NAME tid1 sid1 in
  H5d.write dataset tid1 H5s.all H5s.all wdata;
  H5d.close dataset;
  let dataset = H5d.open_ (H5f.to_loc file) _DSET_VLSTR_NAME in
  ...
  let rdata = Array.make _SPACE1_DIM "" in
